...
def test_parser(parser):
    ...
    args = parser.parse_args("-f a_file".split())
    ...
    args = parser.parse_args(["-lp"])
    assert args.plot == "linespoints"
    args = parser.parse_args(["--linespoints"])
    assert args.plot == "linespoints"

    args = parser.parse_args("-c 1,2 1,3".split())
    ...
    with pytest.raises(SystemExit):
        ...
        parser.parse_args(["--bar", "--scatter"])

    args = parser.parse_args("--xlabel xlabel".split())
    assert args.xlabel == "xlabel"
    args = parser.parse_args("-xl xlabel".split())
    assert args.xlabel == "xlabel"

    args = parser.parse_args("--ylabel ylabel".split())
    assert args.ylabel == "ylabel"
    args = parser.parse_args("-yl ylabel".split())
    assert args.ylabel == "ylabel"

    args = parser.parse_args("--title title".split())
    assert args.title == "title"
    args = parser.parse_args("-t title".split())
    assert args.title == "title"

    args = parser.parse_args("-g".split())
...
def test_get_numerical_data(parser, numerical_data_txt):
    args = parser.parse_args(f"-f {numerical_data_txt} -c 1,2 1,3".split())
    ...
    for p in "-s -b -l -lp".split():
        args = parser.parse_args(f"{args_str} {p}".split())
        data = cli.get_data(args)
...
def test_get_str_data(parser, str_data_txt):
    ...
    args = parser.parse_args(f"-f {str_data_txt} -c 2,1 -b".split())
    data = cli.get_data(args)
...
def test_get_numerical_data_csv(parser, numerical_data_csv):
    ...
    with pytest.raises(IndexError):
        args = parser.parse_args(f"-f {numerical_data_csv}".split())
        data = cli.get_data(args)

    # columns are delimited by "," so we pass "-d ,"
    args = parser.parse_args(f"-f {numerical_data_csv} -c 1,2 1,3 -d ,".split())
    data = cli.get_data(args)
    assert data[0] == [1.0, 2.0, 4.0, 8.0]
    # data[1] == [2.17446, 1.248573, 0.961571, 0.429465]
    for a, b in zip(data[1], [2.17446, 1.248573, 0.961571, 0.429465]):
