...
- To control all axes simultaneously, use the function `frame()` instead, which will show or remove the **plot frame** (composed of all 4 axes); it require a single Boolean.
...
Colors could be also applied to the data markers using the `color` parameter, available in most plotting function.

Colors could easily applied to the rest of the plot, using the `color` parameter of the following methods:
...
Styles could be also applied to the data markers using the `style` parameter, available in most plotting function, including `colorize()`, described [here](https://github.com/piccolomo/plotext/blob/master/readme/utilities.md#colored-text).
...
To quickly chose a favoured color and style combination, for the entire figure or one of its subplots, use the function `theme()`. The available themes could be displayed with the function `themes()`; here is its output: 
