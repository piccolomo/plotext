...
A major rewrite. The flat module-level API (``plt.scatter``, ``plt.show``) is replaced by an explicit object-oriented one built around a singleton master figure (``plt.figure``); plotting calls are methods on it. The rendering kernel is now C++.

API style
^^^^^^^^^
- Singleton master figure: ``fig = plt.figure``; every plotting call is ``fig.<method>(...)``.
- Drawables are first-class: ``fig.signal(...)`` returns a configurable signal, chainable via ``.lines()``, ``.label()``, ``.fillx()``, ``.filly()``, then handed to ``fig.draw(...)``.
- Module-level helpers are limited to a small set: ``sin``, ``square``, ``noise``, ``sleep``, ``image``, ``gif``, ``video``, ``matplotlib``, ``effect``, ``colorize``, ``uncolorize``, and the reference tables (``colors``, ``styles``, ``markers``, ``themes``).

Removed
^^^^^^^
- Module-level axis helpers: ``xlabel``/``ylabel``, ``xlim``/``ylim``, ``xscale``/``yscale``, ``xticks``/``yticks``, ``xfrequency``/``yfrequency``, ``xreverse``/``yreverse``, ``xaxes``/``yaxes``.
- Module-level styling helpers: ``canvas_color``, ``axes_color``, ``ticks_color``, ``ticks_style``, ``clear_color``/``clc``, ``clear_terminal``/``clt``.
- Module-level plot constructors: ``scatter``, ``plot``, ``simple_bar``/``simple_multiple_bar``/``simple_stacked_bar``, ``confusion_matrix``/``cmatrix``, ``indicator``, ``matrix_plot``, ``image_plot``.
- Module-level subplot helpers: ``subplot``, ``subplots``, ``main``, ``active``.
- Module-level media helpers: ``play_gif``, ``play_video``, ``play_youtube``, ``get_youtube`` (replaced by ``plt.gif``, ``plt.video``, ``plt.youtube``).
- ``from_matplotlib`` (replaced by ``plt.matplotlib``).
- ``plot_size`` / ``limit_size`` / ``take_min`` — size management is figure-internal.
- ``save_fig`` (use ``fig.save``).

Renamed
^^^^^^^
- ``scatter`` / ``plot`` → ``fig.signal(...)`` plus ``fig.draw(signal.lines(True/False))``.
- ``xlim``/``ylim`` → ``fig.lim(lower, upper, axis, side)``; same per-axis pattern for ``label``, ``scale``, ``ticks``, ``frequency``.
- ``xreverse``/``yreverse`` → ``fig.direction(direction, axis, side)``.
- ``xaxes``/``yaxes`` → ``fig.axis(...)``.
- ``ticks_color``/``ticks_style`` → ``fig.ruler_pixel(pixel, axis, side)``.
- ``clear_figure`` / ``clf`` → ``fig.clear()``.
- ``colorize``: now a class instead of a function; ``fullground`` parameter renamed ``foreground``; ``show`` parameter removed in favour of ``.print()``.

New
^^^
- Command-line tool — ``plotext`` (and ``python -m plotext``) with a chain-of-methods syntax that auto-tracks the API via introspection. Each ``--method`` opens a call, subsequent tokens are its args, and signal-creating methods enter a drawable-config phase closed by ``--draw``. Highlights:

   - ``@path:<file.csv>`` loads a CSV (with ``:1``, ``:1,2``, or ``:dict`` column-selection suffixes); ``@sample:<name>`` loads a bundled sample (``pizzas``, ``stock``, ``puppy``, ``shaq``) auto-discovered from ``plotext/_examples/data/``.
   - URL inputs route through the path-cache hook (see below), so ``@path:https://…/file.csv``, ``--image https://…``, ``--gif https://…`` and ``--video https://…`` all work.
   - Media methods (``--image``, ``--gif``, ``--video``) bypass the figure pipeline and print directly — no ``--draw``/``--show`` needed.
   - ``-c "<code>"`` runs arbitrary Python with ``plotext`` pre-bound as ``plt`` (for animation loops and anything the chain syntax can't express).
   - ``plotext --methods`` lists every callable method grouped by category; ``plotext --<method> --help`` prints a single docstring.

   See :doc:`cli`.
- URL-aware paths everywhere: any ``plt.image``, ``plt.gif``, ``plt.video`` (and ``fig.image``) call accepts ``http://`` / ``https://`` URLs. URLs are downloaded once into a per-user temp folder (``<tempfile.gettempdir()>/plotext/`` — ``/tmp/plotext/`` on Unix, ``%TEMP%\plotext\`` on Windows) and reused on subsequent calls.
- ``plt.video`` now handles YouTube URLs natively: pass any ``youtube.com`` / ``youtu.be`` URL to ``video`` and it's resolved via ``yt-dlp`` internally. The dedicated ``plt.youtube`` helper has been removed (use ``plt.video``).
- Signal class — ``fig.signal(...)`` with chainable configuration before draw; specialized variants exposed as ``plotext.point`` and ``plotext.points``.
- ``fig.draw(drawable)`` as the universal registration entry point.
- ``fig.theme(name)`` unified preset switching.
- ``fig.box`` (box-and-whisker plot).
- ``fig.line`` / ``fig.segment`` / ``fig.rectangle`` / ``fig.polygon`` shape primitives.
- ``fig.heatmap`` — grid/colormap rendering of a 2D matrix (replaces module-level ``matrix_plot`` / ``image_plot``).
- ``fig.confusion_matrix`` — cross-tab grid with auto-normalized cell labels and per-cell colormap (replaces module-level ``confusion_matrix`` / ``cmatrix``).
- ``fig.candlestick`` — OHLC chart from a single dict with ``date`` / ``open`` / ``close`` / ``high`` / ``low`` keys; ``style='ohlc'`` switches the body to ticks for dense series (was a positional-args API in 5.x).
- ``fig.legend(...)`` with relative-position controls.
- ``fig.tick_alignment``, ``fig.ruler_pixel``.
- ``fig.interactive`` on the figure (replaces module-level ``interactive``).
- ``fig.time(full)`` for render-timing introspection.
- ``plotext.effect`` — phase-driven text animations (shimmer, rainbow, gradient, pulse).
- ``plotext.sleep`` — pause-between-frames helper for streaming plots.
