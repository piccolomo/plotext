...
- :meth:`~plotext._plotter.plot.plot_class.clear_size` (alias ``clz``) — drops any explicit *plot_size* value and resets every subplot's size to ``None`` so the next harmonization redistributes proportionally. On the master it also calls :meth:`~plotext._kernel.terminal.terminal.clear` so the terminal's sizing state (prompt height + per-axis limit flags, see :doc:`size`) returns to its defaults and the master size snaps back to the current terminal dimensions. Signals, subplots, settings, pixels and styles are preserved.
...
Calling :meth:`~plotext._kernel.terminal.terminal.clear` directly only resets the terminal object — the figure's signals and layout are not touched. The reverse direction *does* cascade: the figure's :meth:`~plotext._plotter.plot.plot_class.clear` (and :meth:`~plotext._plotter.plot.plot_class.clear_size`) reach into the terminal and reset its sizing state, so a single ``fig.clear()`` leaves no sticky terminal-level state from a previous run.
