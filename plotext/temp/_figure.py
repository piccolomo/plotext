...
#         self.monitor = monitor_class(self.date) if self._is_master else self._parent.monitor.copy() # each figure has a monitor for plotting; which by default is deep copied from its parent figure monitor, so that a figure with multiple sub plots can have easily the same plot and plot settings and preferences (without rewriting code)
...
#         self._interactive = default_figure.interactive if interactive is None else bool(interactive)
...
#         self._limit_width = default_figure.limit_width if width is None else bool(width)
#         self._limit_height = default_figure.limit_height if height is None else bool(height)
#         self._limit = [self._limit_width, self._limit_height]

#     def _set_terminal_size(self, width = None, height = None):
#         self._width_term = default_figure._width_term if width is None else width
#         extra_lines = 2 if ut.is_ipython() else 1
#         self._height_term = default_figure._height_term if height is None else max(height - extra_lines, 0)
...
#     def candlestick(self, dates, data, positive_color = None, negative_color = None, orientation = None, xside = None, yside = None, label = None):
#         self.monitor.draw_candlestick(dates, data, xside = xside, yside = yside, orientation = orientation, positive_color = positive_color, negative_color = negative_color, label = label) if self._no_plots else [[self._get_subplot(row, col).candlestick(dates, data, orientation = orientation, colors = colors, label = label) for col in self._Cols] for row in self._Rows]

#     def box(self, *args, quintuples = None, color = None,  fill = None, width = None, orientation = None, minimum = None, reset_ticks = None, xside = None, yside = None, label = None):
#         self.monitor.draw_box(*args, xside = xside, yside = yside, orientation = orientation, color = color, label = label, fill = fill, width = width, minimum = minimum, reset_ticks = reset_ticks, quintuples = quintuples) if self._no_plots else [[self._get_subplot(row, col).box(*args, orientation = orientation, color = color, label = label, fill = fill, width = width, minimum = minimum, reset_ticks = reset_ticks, quintuples = quintuples) for col in self._Cols] for row in self._Rows]
...
#     def strings_to_times(self, string, input_form = None):
