...
public:
    ...
    // Deduplicate overlapping points. Walks each Point: if the (col, row) cell already holds an index, mark the previous Point as null when the new one is_close (same sub-cell). After the pass, drop nulled entries. Uses PointsGrid for O(1) (col, row) → previous-index lookup.
    inline void squash(PointsGrid & grid) noexcept {
        ...
        for (Point & current : *this) {
            ...
            if (grid.is_present(c, r)) {
                ...
                if (current.is_close(previous)) previous.set_marker(nullptr); }
...
extern "C" {
    ...
    void     points_squash           (Points * ps, PointsGrid * g) noexcept { ps->squash(*g); }
}
