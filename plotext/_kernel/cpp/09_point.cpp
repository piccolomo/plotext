// Point: a 2D position (inherits PointPosition) carrying an OWNED Marker (deep-copied via Marker::copy on construction/copy). Owning lifetime keeps Point usable independently of any Python/marker handle being garbage-collected.

class Point : public PointPosition {
private:
    Marker * marker = nullptr;        // owned — destroyed in ~Point, deep-copied in copy paths

public:
    Point() noexcept = default;
    Point(float x, float y, Marker * m = nullptr) noexcept : PointPosition(x, y), marker(m ? m->copy() : nullptr) {}
    ~Point() noexcept { delete marker; }

    Point(const Point & o) noexcept : PointPosition(o), marker(o.marker ? o.marker->copy() : nullptr) {}
    Point(Point && o) noexcept : PointPosition(o), marker(o.marker) { o.marker = nullptr; }
    Point & operator=(const Point & o) noexcept {
        if (this != &o) { delete marker; PointPosition::operator=(o); marker = o.marker ? o.marker->copy() : nullptr; }
        return *this; }
    Point & operator=(Point && o) noexcept {
        if (this != &o) { delete marker; PointPosition::operator=(o); marker = o.marker; o.marker = nullptr; }
        return *this; }
    ...
    inline size_t   get_foreground_integer_code() const noexcept { return marker ? marker->get_foreground_integer_code() : no_color; }   // null-safe — returns no_color sentinel when no marker

    // Sub-cell dot coords inside the marker glyph (fractional part of x/y scaled by marker grid)
    inline unsigned char get_dot_col() const noexcept { return marker ? static_cast<unsigned char>((get_x() - get_col()) * marker->get_cols()) : 0; }
    inline unsigned char get_dot_row() const noexcept { return marker ? static_cast<unsigned char>((get_y() - get_row()) * marker->get_rows()) : 0; }
    ...
    inline size_t get_x_distance_to(const Point & p) const noexcept {
        const size_t cols = marker ? marker->get_cols() : 1;
    ...
    inline size_t get_y_distance_to(const Point & p) const noexcept {
        const size_t rows = marker ? marker->get_rows() : 1;
    ...
    inline size_t get_cols() const noexcept { return marker ? marker->get_cols() : 1; }
    inline size_t get_rows() const noexcept { return marker ? marker->get_rows() : 1; }
    ...
    inline bool is_close(const Point & p) const noexcept {
        return get_type()    == p.get_type()
            ...
            && get_dot_col() == p.get_dot_col() && get_dot_row() == p.get_dot_row(); }
    ...
    inline Vector<Point> get_full_line(const Point & p, bool last = true) const noexcept {
        const float cols = static_cast<float>(get_cols());
        const float rows = static_cast<float>(get_rows());
    ...
    inline MatrixCharacter get_matrix_character() const { return marker ? marker->get_matrix_character(get_dot_col(), get_dot_row()) : MatrixCharacter(); }
