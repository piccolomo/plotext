...
public:
    ...
    // Copy constructor
    constexpr MatrixBool(const MatrixBool &) noexcept = default;

    // Move constructor
    constexpr MatrixBool(MatrixBool &&) noexcept = default;

    // Virtual destructor for safe polymorphic deletion (CharacterHD inherits)
    virtual ~MatrixBool() noexcept {}

    // Copy assignment
    constexpr MatrixBool & operator=(const MatrixBool &) noexcept = default;

    // Move assignment
    constexpr MatrixBool & operator=(MatrixBool &&) noexcept = default;
    ...
    // Copy only the size (cols and rows) from another MatrixBool
    ...
    // OR-accumulate another MatrixBool's bits into this one
