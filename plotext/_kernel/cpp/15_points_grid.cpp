// PointsGrid: 2D grid mapping (col, row) → point index in some Points collection. Cell stores index+1; 0 means empty (saves a sentinel value). Used by Points::squash for fast O(1) overlap lookup.

class PointsGrid : public Vector<size_t> {
private:
    size_t cols  = 0;
    size_t rows  = 0;
...
public:
    PointsGrid(size_t c, size_t r) { resize(c, r); }
    ~PointsGrid() noexcept = default;
    ...
    wstring get_wstring() const noexcept {
        wostringstream os;
        os << L"PointsGrid " << cols << L"x" << rows << L" count " << count << L" [";
...
extern "C" {
    PointsGrid * points_grid_new       (size_t cols, size_t rows) noexcept { return new PointsGrid(cols, rows); }
    void         points_grid_delete    (PointsGrid * g) noexcept { delete g; }
    void         points_grid_clear     (PointsGrid * g) noexcept { g->clear(); }
    void         points_grid_log       (PointsGrid * g) noexcept { g->log(); }
    size_t       points_grid_get_length(PointsGrid * g) noexcept { return g->get_count(); }
}
