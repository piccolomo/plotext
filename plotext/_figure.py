...
class figure_class():
    ...
    def set_size(self, width = None, height = None):
        self.width = None if width == None else int(width)
        self.height = None if height == None else int(height)
        self.size = [self.width, self.height]
        
    def set_subplots(self, rows = None, cols = None): # it sets the figure grid size
        rows = 1 if rows == None else rows
        cols = 1 if cols == None else cols
    ...
    def set_subplot(self, row = None, col = None): # it sets the current subplot to work on
        row = 1 if row == None else row
        col = 1 if col == None else col
...
##############################################
    ...
    def plot_size(self, width = None, height = None):
        width = None if width == None else width 
        height = None if height == None else height
    ...
    def limitsize(self, limit_xsize = None, limit_ysize = None): # can't call it limit_size here bacause of internal parameter named the same
        limit_xsize = self.default.limit_size[0] if limit_xsize == None else bool(limit_xsize)
        limit_ysize = limit_xsize if limit_ysize == None else bool(limit_ysize)
        self.limit_size = [limit_xsize, limit_ysize]

    def span(self, colspan = None, rowspan = None):
        colspan = 1 if colspan == None or colspan  <= 0 else colspan
        rowspan = 1 if rowspan == None or rowspan  <= 0 else rowspan
    ...
    def ylabel(self, ylabel = None, yside = None):
        ylabel = None if ylabel == None else str(ylabel).strip()
    ...
    def xaxis(self, state = None, xside = None):
        pos = self.subplot.xside_to_pos(xside)
        state = self.subplot.default.xaxes[pos] if state == None else bool(state)
        self.subplot.xaxes[pos] = state

    def yaxis(self, state = None, yside = None):
        pos = self.subplot.yside_to_pos(yside)
        state = self.subplot.default.yaxes[pos] if state == None else bool(state)
    ...
    def grid(self, horizontal = None, vertical = None):
        horizontal = self.subplot.default.grid[0] if horizontal == None else bool(horizontal)
        vertical = horizontal if vertical == None else bool(vertical)
        self.subplot.grid = [horizontal, vertical]

    def canvas_color(self, color = None):
        code = color_code(color, 0)
        nocolor = code[0] == 3
        color = self.subplot.default.canvas_color if color == None or nocolor else color
        self.subplot.canvas_color = color

    def ticks_color(self, color = None):
        code = color_code(color, 1)
        nocolor = code[0] == 3
        color = self.subplot.default.ticks_color if color == None or nocolor else color
        self.subplot.ticks_color = color
        
    def axes_color(self, color = None):
        code = color_code(color, 0)
        nocolor = code[0] == 3
        color = self.subplot.default.axes_color if color == None or nocolor else color
    ...
    def ylim(self, left = None, right = None, yside = None):
        left = None if left == None else float(left)
        right = None if right == None else float(right)
        ylim = [left, right]
    ...
    def xscale(self, scale = None, xside = None):
        default_case = (scale == None or scale not in self.subplot.default.xscale)
    ...
    def yscale(self, scale = None, yside = None):
        default_case = (scale == None or scale not in self.subplot.default.yscale)
    ...
    def xfrequency(self, frequency = None, xside = None):
        pos = self.subplot.xside_to_pos(xside)
        frequency = self.subplot.default.xfrequency[pos] if frequency == None else int(frequency)
        if frequency == 0:
            self.xticks([], [], xside)
    ...
    def yfrequency(self, frequency = None, yside = None):
        pos = self.subplot.yside_to_pos(yside)
        frequency = self.subplot.default.yfrequency[pos] if frequency == None else int(frequency)
        if frequency == 0:
            self.yticks([], [], yside)
    ...
    def xticks(self, ticks = None, labels = None, xside = None):
        pos = self.subplot.xside_to_pos(xside)
        ticks = self.subplot.default.xticks[pos] if ticks == None else list(ticks)
        labels = ticks if labels == None else list(labels)
        ...
        self.subplot.xfrequency[pos] = self.subplot.xfrequency[pos] if ticks == None else len(ticks) 

    def yticks(self, ticks = None, labels = None, yside = None):
        pos = self.subplot.yside_to_pos(yside)
        ticks = self.subplot.default.yticks[pos] if ticks == None else list(ticks)
        labels = ticks if labels == None else list(labels)
        labels = list(map(str, labels))
        ticks, labels = brush(ticks, labels)
        self.subplot.yticks[pos] = ticks
        self.subplot.ylabels[pos] = labels
        self.subplot.yfrequency[pos] = self.subplot.yfrequency[pos] if ticks == None else len(ticks)
