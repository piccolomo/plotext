...
doc("Reads the full text contents of a file.")
par("path", "File path"); spec(type.string)
out("File contents", type.string)

add(file.write, name = "file.write")
doc("Writes text to a file. Pass `append=True` to append instead of overwriting.")
par("text", "Text to write"); spec(type.string)
par("path", "File path"); spec(type.string)
