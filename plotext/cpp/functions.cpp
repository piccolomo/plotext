...
template <typename T>
inline vector<T> split_string(const T & str, typename T::value_type separator = ' ') noexcept {
    using CharT = typename T::value_type;
    basic_stringstream<CharT> iss(str);
    vector<T> tokens;
    T token;
    while (getline(iss, token, separator)) {tokens.push_back(token);}
    return tokens;}
