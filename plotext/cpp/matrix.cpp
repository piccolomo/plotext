...
public:
  inline Matrix(const size_t & w, const size_t & h) noexcept : strings(new String[h]), height(h) {for (int row = 0; row < height; row++) {String * address = &strings[row]; new(address) String(w);}}
  inline Matrix(const size_t & w, const size_t & h, const Pixel & p) noexcept : Matrix(w, h) {fill_pixel(p);}
  inline Matrix(const wstring & str, const Pixel & p = Pixel()) noexcept {
    ...
    this->height = h;
    this->strings = new String[h];
    for (size_t row = 0; row < h; row++) {
      wstring s = wstrings.at(row);
      s.resize(width, L' ');
      new (&this->strings[row]) String(s, p);}}
  inline ~Matrix() noexcept {delete [] strings; strings = nullptr;}

  inline constexpr void clear(){for (size_t row = 0; row < height; row ++){strings[row].clear();}}

  inline size_t get_length() const noexcept {size_t size = 0; for(size_t row = 0; row < height; row++){size += get_string().size();} return size;}
  ...
  inline void resize(const size_t & new_width, const size_t & new_height) noexcept {
    Matrix novel(new_width, new_height);
    Matrix reshaped = part(min(get_width(), new_width), min(height, new_height));
    novel.insert(0, 0, reshaped);
    this->operator=(novel);}
  ...
  inline void vstack(const Matrix & m, const bool & adapt = false) noexcept {
    size_t new_width = adapt? max(get_width(), m.get_width()) : get_width();
      size_t old_height = height;
      resize(new_width, old_height + m.get_height());
      insert(0, old_height, m);}

  inline void hstack(const Matrix & m, const bool & adapt = false) noexcept {
  ...
  inline wstring get_string(const bool & colorless = false) const noexcept {
  ...
  inline void show(const bool & colorless = false) const noexcept {wcout << get_string(colorless) << endl;}
};
