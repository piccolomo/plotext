...
public:
  ...
  inline Points(const size_t & size) noexcept {points.reserve(size);};

  inline size_t get_length() const noexcept {return points.size();}
  inline Point & at(const size_t & i) noexcept {return points.at(i);}

  inline void add(const float & xi, const float & yi, const Marker & m, const MarkerInfo & mi = MarkerInfo()) noexcept {points.emplace_back(xi, yi, m, mi);}
  ...
  inline void add_lines() noexcept {
      ...
      float dx = 1. / at(i).get_cols(); float dy = 1. / at(i).get_rows(); float ds = dy / dx;
    	float sx = Dx > 0 ? dx : -dx; float sy = Dy > 0 ? dy : -dy; 
    	MarkerInfo mi(0, at(i).get_fillx(), at(i).get_filly());
