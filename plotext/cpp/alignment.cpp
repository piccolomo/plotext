...
// inline int _get_displacement(const size_t & width, const unsigned & ha) noexcept {if (ha == -1) {return 0;} else if (ha == 0) {return - (width - 1) / 2;} else {return 1 - width;}}

// inline vector<int> get_dynamic_displacements(const size_t & width) noexcept {vector<int> unsorted = range<int>(_get_displacement(width, 1), 1); return sort(unsorted, _get_displacement(width, 0));}
