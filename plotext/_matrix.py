...
class matrix_class():
    ...
    def align_col(self, col, length, alignement):
        integer_alignment = get_horizontal_alignment_index(alignement)
        delta = [0, - length // 2 , - length + 1]
        return col + delta[integer_alignment]

    # def check_row_space(self, col, row, rows):
    #     return matrix_check(self.pointer, col, row, rows)

    def check(self, col, row, cols, rows):
        return matrix_check(self.pointer, col, row, cols, rows)
        #return all([self.check_row_space(col, row, rows) for col in range(col, col + cols)])
    ...
    def check_matrix(self, col, row, matrix):
        ...
        #res = res if row == 0 else res and self.check(col, row - 1, cols, 1)
        #res = res if row == self.rows() - rows else res and self.check(col, row + rows, cols, 1)
        return res
