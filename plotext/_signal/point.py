...
class point_class:
    # Initialize from (x, y, marker) or an existing C pointer. marker may be marker_class, matrix_marker, box_class (or line subclass), or anything coercible to marker_class — each of these wraps a C++ Marker*.
    def __init__(self, x = 0, y = 0, marker = None, _pointer = None):
        if marker is None:                                                              marker = marker_class()
        elif not isinstance(marker, (marker_class, matrix_marker_class, box_class)):    marker = marker_class(marker)
    ...
    def __repr__(self):
        return f"Plotext Point: " + self.get_string()
